...
am5.ready(function () {
    ...
    // Function to update broker status
    ...
    // Function to add MQTT message to live feed
    function addMQTTMessage(topic, payload) {
        ...
        messageDiv.className = topic.includes('temperature') ? 'message-item temp' : 'message-item hum';

        const now = new Date();
        const timeString = now.toLocaleTimeString();

        const icon = topic.includes('temperature') ? '🌡️' : '💧';
        const unit = topic.includes('temperature') ? '°C' : '%';

        messageDiv.innerHTML = `
            <div class="message-header">
                <span class="message-topic">${icon} ${topic}</span>
                <span class="message-time">${timeString}</span>
            </div>
            <div class="message-payload">Payload: ${payload}${unit}</div>
        `;
    ...
    document.getElementById('pushbuttonPower').addEventListener('click', function () {
        ...
        if (image_tracker === "dis") {
            ...
            messageInterval = setInterval(() => {
                addMQTTMessage('iot/temperature', '24');
                setTimeout(() => {
                    addMQTTMessage('iot/humidity', '40');
                }, 500);
            }, 2000);

            alert("Data sending using MQTT protocol. Check IoT Server (MQTT Broker)");
        } else {
            ...
            // Stop updating charts and messages
            ...
            // Clear messages
            const messagesContainer = document.getElementById('messagesContainer');
            messagesContainer.innerHTML = '<div class="no-messages">No messages yet. Start simulation to see published data...</div>';
